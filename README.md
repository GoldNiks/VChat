...
- Текущий этап развития игрока (LV, MV, UV и т.п.) по конкретным выполненным квестам FTB Quests в суффиксе TAB или чата.
...
```json5
{
  ...
  "stages": {
    "enabled": true,
    "detectionMode": "started",
    "appendToSuffix": true,
    "separator": " ",
    "quests": [
      // Замените примеры ID на ID нужных квестов из ваших .snbt-файлов.
      { "questId": "0123456789ABCDEF", "tag": "&7Stone Age" },
      { "questId": "FEDCBA9876543210", "tag": "&7Steam" }
    ],
    "chapters": [
...
Если FTB Quests установлен и `stages.enabled: true`, заполните `stages.quests` конкретными ID квестов. Суффикс выдаётся после выполнения квеста; если выполнено несколько указанных квестов, используется последний в списке. ID — это поле `id` самого квеста внутри файла `config/ftbquests/quests/chapters/*.snbt`, а не первое поле файла, относящееся к главе. Как только `quests` заполнен, резервный список `chapters` не используется. При `appendToSuffix: true` тег автоматически дописывается в конец `<suffix>` в TAB и чате; иначе его можно вывести через `<stage>`.
...
Во всех игровых форматах VChat можно использовать оба вида: `<name>` и `%name%`. В `globalFormat`, `localFormat`, `playerFormat`, header и footer доступны:
...
| `<player>` | Текущее отображаемое имя (совместимый псевдоним) |
| `<group>` | Primary group LuckPerms |
| `<world>` | Текущее измерение |
| `<channel>` | `global`, `local` или `tab` |
| `<message>` | Текст сообщения |
| `<stage>` | Текущий этап развития игрока по выполненным квестам из `stages.quests` |
...
Они работают в header/footer, `joinMessage` и `firstJoinMessage`. Для каждого можно использовать угловой или процентный вид. `<message>`/`%message%` имеет содержимое только в форматах чата; в остальных местах заменяется пустой строкой.

## VEconomy: баланс игрока

Если VEconomy установлен, `<balance>` или `%balance%` показывают баланс игрока в том же виде, что и команда VEconomy (символ валюты, разделители, склонение). Интеграция необязательная и работает через публичный API VEconomy: без мода плейсхолдер заменяется пустой строкой.
