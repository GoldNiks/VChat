...
В header/footer: `%online%`, `%max%`, `%player%`, `%balance%`, `%tps%`. В `firstJoinMessage` доступен `<name>`.

## VEconomy: баланс игрока

Если VEconomy установлен, `<balance>` в `playerFormat`/форматах чата и `%balance%` в header/footer показывают баланс игрока в том же виде, что и команда VEconomy (символ валюты, разделители, склонение). Интеграция необязательная и работает через публичный API VEconomy: без мода плейсхолдер заменяется пустой строкой.
...
1. Поместить `VChat-1.6.9.jar` в `mods/` сервера Forge 1.20.1.
